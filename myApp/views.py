...
def company(request):
    ...
    pieData = getCompanyCharData.pieData(type)
...
def educational(request):
    ...
    defaultEducation = '不限'
    if request.GET.get("educational"):defaultEducation = request.GET.get("educational")
    educations,charDataRow,chrDataColumn1,chrDataColumn2,hasEmpty = getEducationalCharData.getExpirenceLineData(defaultEducation)
...
def address(request):
    ...
    edcationData = getAddressCharData.getEducation(defaultAddress)
    distData = getAddressCharData.getDist(defaultAddress)
    salaryList,salaryColumn = getAddressCharData.getSalary(defaultAddress)
    peopleData = getAddressCharData.companyPeopleData(defaultAddress)

    randomIntNumber = random.randint(1,100000)
    get_addressCompanyTags_img(r'.\static\3.jpg', f'.\static\companyAddressTags{randomIntNumber}.jpg',defaultAddress)

    return render(request,'address.html',{
        'userInfo':userInfo,
        'addressList': addressList,
        'defaultAddress':defaultAddress,
        'edcationData':edcationData,
        'distData':distData,
        'salaryList':salaryList,
        'salaryColumn':salaryColumn,
        'peopleData':peopleData,
        'randomIntNumberImg':randomIntNumber
    })